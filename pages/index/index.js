...
Page({
  ...
  getList(id) {
    ...
    dbJs.query("square_data", { type_id: _.eq(id) }).then((res) => {
      ...
      data.forEach((item, index) => {
        ...
        item.formatRelease = toolJs.formatTime(release);
        item.formatRelease0 = toolJs.formatTime(release, true);
        ...
        console.log("getList-hasStatus", !!hasStatus);
        item.isLike = !!hasStatus.status;
        // this.setData({
        //   [`dataList[${index}].isLike`]: !!hasStatus.status,
        // });
      });
      this.setData({
        dataList: data,
  ...
  clickType(e) {
    ...
    if (this.data.currentType === idx) return false;
    this.setData({
      currentType: idx,
    });
  ...
  clickSmallType(e) {
    ...
    if (this.data.currentSmallType === idx) return false;
    this.setData({
  ...
  zan(e) {
    ...
    commonJs.thumbsUp(obj).then((res) => {
      ...
      this.setData({
        ...
        [`dataList[${index}].like_num`]: obj.status ? --likeNum : ++likeNum,
      });
      this.getZanList();
    });
  },
